...
#[inline(always)]
fn luminance_to_lstar(y: f32) -> f32 {
...
/// Consider using SIMD once it's stable
pub fn fast_lstar(
    bgr: &[u8],
    gamma_lut: &[f32; 256],
    lstar_lut: &[f32; 4096],
    ...
    for (i, chunk) in bgr.chunks_exact(3).enumerate() {
        // sRGB → linear
        let r_lin = gamma_lut[chunk[2] as usize];
        let g_lin = gamma_lut[chunk[1] as usize];
        let b_lin = gamma_lut[chunk[0] as usize];
        // luminance (Y)
        let y = 0.2126 * r_lin + 0.7152 * g_lin + 0.0722 * b_lin;
        // L*
        // let lstar = luminance_to_lstar(y);
        let idx = (y * (lstar_lut.len() as f32 - 1.0)) as usize;
        out[i] = lstar_lut[idx] as u8;
    }
}
...
fn detect_cells(
    ...
    let lut2 = build_lstar_lut();
    let bgr = bit_mat.data_bytes()?;
    let mut l = Vec::new();
    let start = std::time::Instant::now();
    fast_lstar(bgr, &lut, &lut2, &mut l);
	let end = std::time::Instant::now();
	println!("L* Took: {} seconds", (end - start).as_secs_f32());
    let lmat = Mat::new_rows_cols_with_bytes::<u8>(
        temp_mat.rows(),
        temp_mat.cols(),
        &l
    )?;
    let mut new = temp_mat.clone();
    //insert_channel(&lmat, &mut new, 0)?;
    opencv::imgcodecs::imwrite_def("dichromate/pics2/T_devi.png", &new).unwrap();
    std::mem::swap(temp_mat, &mut new);
    ...
    // Get area we want to keep and invert it
    imgproc::threshold(
        write_mat,
        thresh_mat,
        40.,
    ...
    temp_mat.copy_to_masked(hsvthresh_mat, thresh_mat)?;
    ...
    hsv_mat.copy_to_masked(&mut x, temp_mat)?;
    if debug_info.edges_mat {
        let mut m = Mat::default();
    ...
    // Consider moving blur removal to here
    ...
    temp_mat.copy_to_masked(write_mat, blob_mat)?;
    // rn, blob = good
    opencv::imgcodecs::imwrite_def("dichromate/pics2/T_gray_zcp0.png", blob_mat).unwrap();
    // Whatever we don't keep (whatever didnt make the in range)
    // should also be removed from the (how to do this)
    // look for items in temp but not in blob
    {
        let mut a = Mat::default();
        let mut b = Mat::default();
        let mut c = Mat::default();
        let mut d = Mat::default();
        // everything not in blob
        bitwise_not_def(blob_mat, &mut a)?;
        // copy temp (circle + blur) to b, mask of not circles (blur)
        temp_mat.copy_to_masked(&mut b, &a)?;
        //subtract_def(&x, &b, &mut a)?;
        //extract_channel(
        //    &mut b,
        //    &mut a,
        //    2
        //)?;
        // in b and not in a
        opencv::imgcodecs::imwrite_def("dichromate/pics2/T_gray_zcpa.png", &b).unwrap();
        // use b as mask over the orig
        hsv_mat.copy_to_masked(&mut c, &b)?;
        // get absdfif
        absdiff(&b, &c, &mut d)?;
        // c is what we should remove from hsv and hsvthresh
        in_range(
            &d,
        ...
        a.set_scalar(0.into())?;
        hsv_mat.copy_to_masked(&mut a, &mut c)?;
        opencv::imgcodecs::imwrite_def("dichromate/pics2/T_gray_zcp.png", &a).unwrap();
        std::mem::swap(hsv_mat, &mut a);
        a.set_scalar(0.into())?;
        hsvthresh_mat.copy_to_masked(&mut a, &mut c)?;
        std::mem::swap(hsvthresh_mat, &mut a);
    }
    std::mem::swap(temp_mat, write_mat);
    if debug_info.edges_mat {
        let mut m = Mat::default();
        imgproc::cvt_color(
            temp_mat, 
