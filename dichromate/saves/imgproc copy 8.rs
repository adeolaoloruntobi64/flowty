...
impl CellDetector {
    ...
    pub fn detect_cells2(
        ...
        // Note that lmat references lbytes, lmat must be dropped before or
        // at the same time as lbytes.
        let (lbytes, lmat) = create_lightness_mat(bit_mat, &self.gamma_lut, &self.lstar_lut)?;
        let (lowerthresh, middlethresh, upperthresh) = create_thresholds(&hsv_mat, &lmat)?;
        ...
        let (nhsv, hue_alt, inverted_features) = remove_and_isolate_hue(&hsv_upper, &upperthresh, &dilated_circles, &self.circle_kernel, self.border_value)?;
        opencv::imgcodecs::imwrite_def("dichromate/pics2/U11.png", &nhsv).unwrap();
        // create mask for where nshv != 0 0 0
        let mut dst = Mat::default();
        in_range(&nhsv, &Vec3b::from_array([0, 0, 1]), &Vec3b::from_array([255, 255, 255]),  &mut dst)?;
        ...
        let stats = stats_with_mask(&hue_alt, &dst)?;
        //opencv::imgcodecs::imwrite_def("dichromate/pics2/U5.png", &upperthresh).unwrap();
        opencv::imgcodecs::imwrite_def("dichromate/pics2/U6.png", &dst).unwrap();
        println!("STATS: {stats:?}");
        // The board without dots or bg (some dots might have the same hue so remove them)
        let bare_board = apply_stats(stats, &hsv_lower, &inverted_features)?;
        let new_features = get_newly_added_features(&dst, &bare_board)?;
        let final_prod= delete_select_features(&new_features, &bare_board, &self.circle_kernel, self.border_value)?;
...
fn create_lightness_mat(bit_mat: &Mat, gamma_lut: &[f32; 256], lstar_lut: &[u8; 4096]) -> opencv::Result<(Vec<u8>, Mat)> {
    let bgr = bit_mat.data_bytes()?;
    let mut lightness = Vec::new();
    fast_lstar(bgr, gamma_lut, lstar_lut, &mut lightness);
    let lmat = unsafe {
        Mat::new_rows_cols_with_data_unsafe(
            bit_mat.rows(),
            bit_mat.cols(),
            CV_8UC1,
            lightness.as_ptr() as *mut _,
            Mat_AUTO_STEP
        ).unwrap()
    };
    return Ok((lightness, lmat))
}

fn create_thresholds(hsv_mat: &Mat, lmat: &Mat) -> opencv::Result<(Mat, Mat, Mat)> {
    ...
    bitwise_and_def(&combined1, &keep3, &mut middlethresh)?;
...
fn remove_and_isolate_hue(hsv_upper: &Mat, upperthresh: &Mat, dilated_circles: &Mat, circle_kernel: &Mat, border_value: Scalar) -> opencv::Result<(Mat, Mat, Mat)> {
    ...
    Ok((hsv_reformed, hue_component, inverted_features))
}
...
fn apply_stats(
    ...
    let (lower_hue, lof) = mode.overflowing_sub(7);
    let (upper_hue, uof) = mode.overflowing_add(7);
    if !lof && !uof  {
        println!("NO OVERFLOW");
        in_range(
            hsv_lower,
        ...
        bitwise_or(&keep, &keep, &mut lvl, &inverted_features)?;
    } else if lof && uof {
        ...
        bitwise_or(&keep, &keep2, &mut lvl, &inverted_features)?;
    }
    Ok(lvl)
}
...
fn delete_select_features(new_features: &Mat, bare_board: &Mat, circle_kernel: &Mat, border_value: Scalar) -> opencv::Result<Mat> {
    let mut dst = Mat::default();
    ...
    let slight_kernel = get_structuring_element(
        MORPH_ELLIPSE, Size::new(3, 3), Point::new(-1, -1)
    )?;
    morphology_ex(
        new_features,
        &mut dst,
