...
fn mirror_quadrants(coords: &[(i32, i32)]) -> Vec<(i32, i32)> {
    coords.into_iter()
        .flat_map(|(x, y)| {
            // Create combinations of [1, -1] x [1, -1]
            [1, -1].iter().cartesian_product([1, -1].iter())
                .map(move |(&sx, &sy)| (x * sx, y * sy))
        })
        .unique() // Optional: prevents duplicates if any coord is (0,0)
        .collect()
}
