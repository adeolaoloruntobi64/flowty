...
impl CellDetector {
    ...
    pub fn detect_cells2(
        ...
        let lines = repair_lines(&non_zero)?;
        opencv::imgcodecs::imwrite_def("dichromate/pics2/U20.png", &lines).unwrap();
...
fn repair_lines(src: &Mat) -> opencv::Result<Mat> {
    // 1. Pre-process: Ensure binary white-on-black
    let gray = src;
    
    let mut binary = Mat::default();
    threshold(&gray, &mut binary, 127.0, 255.0, THRESH_BINARY)?;

    // 2. Simple 'Bridge' using Morphology (The "Fast" fix)
    // This connects gaps smaller than 5 pixels immediately.
    let mut repaired = Mat::default();
    let kernel = get_structuring_element(MORPH_RECT, Size::new(3, 3), Point::new(-1, -1))?;
    morphology_ex(&binary, &mut repaired, MORPH_CLOSE, &kernel, Point::new(-1, -1), 1, BORDER_CONSTANT, Scalar::default())?;

    // 3. Trajectory Connection (The "Reliable" fix)
    // We look for endpoints and draw a line to the nearest neighbor in front of it.
    let mut final_output = repaired.clone();
    let rows = repaired.rows();
    let cols = repaired.cols();

    for y in 1..rows - 1 {
        for x in 1..cols - 1 {
            let pixel = *repaired.at_2d::<u8>(y, x)?;
            if pixel == 255 {
                if is_endpoint(&repaired, x, y)? {
                    // Search in a small radius (e.g., 10 pixels) for another line
                    if let Some(target) = find_connection_target(&repaired, x, y, 10)? {
                        println!("{:?} -> {target:?}", Point::new(x, y));
                        line(
                            &mut final_output,
                            Point::new(x, y),
                            target,
...
// Helper: An endpoint in a binary image has exactly one neighbor
fn is_endpoint(img: &Mat, x: i32, y: i32) -> opencv::Result<bool> {
    let mut neighbors = 0;
    for i in -1..=1 {
        for j in -1..=1 {
            if i == 0 && j == 0 { continue; }
            if *img.at_2d::<u8>(y + i, x + j)? == 255 {
                neighbors += 1;
            }
        }
    }
...
// Helper: Look in the neighborhood for a pixel that isn't the current line
fn find_connection_target(img: &Mat, x: i32, y: i32, radius: i32) -> opencv::Result<Option<Point>> {
    for r in 2..radius { // Start at 2 to skip the immediate neighbor
        for i in -r..=r {
            for j in -r..=r {
                let nx = x + j;
                let ny = y + i;
                if nx >= 0 && nx < img.cols() && ny >= 0 && ny < img.rows() {
                    if *img.at_2d::<u8>(ny, nx)? == 255 {
                        return Ok(Some(Point::new(nx, ny)));
                    }
                }
            }
        }
